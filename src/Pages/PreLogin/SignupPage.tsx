...
import SignupAnimation from "../../Components/Animations/AnimationSignup.json";
...
export default function SignupPage() {
  ...
  const handleSignup = async () => {
    ...
    try {
      ...
      const userToSend = {
        ...user,
        rentalUnit: Number(user.rentalUnit),
        zipCode: Number(user.zipCode),
        phoneNumber: Number(user.phoneNumber),
      };
      const response = await addUser(userToSend);
  ...
  return (
    ...
    >
      <View style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={styles.scrollContainer} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
          {loading ? (
            <View style={styles.lottieContainer}>
              <LottieView source={SignupAnimation} autoPlay loop style={{ width: 180, height: 180 }} />
              <Text style={{ marginTop: 16, fontSize: 16, color: "#222" }}>Opretter bruger...</Text>
            </View>
          ) : (
            <>
        ...
        <LinearGradient colors={["transparent", "#f9f9f9"]} style={styles.gradient} pointerEvents="none" />
      </View>
    </KeyboardAvoidingView>
  );
}
